...
interface LineCache {
    ...
    u_series_count: WebGLUniformLocation | null;
    u_gradient_lut: WebGLUniformLocation | null;
    a_start: number;
    a_end: number;
    a_series_start: number;
    a_series_end: number;
    a_corner: number;
}
 ...
 * same-series points. Reads the shared `a_color_value` buffer as the
 * per-point series index; segments whose endpoints straddle a series
 * boundary (or land in unused slots tagged with `-1`) are discarded by
 * the vertex shader.
 */
export class LineGlyph implements Glyph {
    ...
    ensureProgram(
        ...
        const cache: LineCache = {
            ...
            u_series_count: gl.getUniformLocation(program, "u_series_count"),
            ...
            a_series_start: gl.getAttribLocation(program, "a_series_start"),
            a_series_end: gl.getAttribLocation(program, "a_series_end"),
    ...
    draw(
        ...
        gl.uniform1f(cache.u_series_count, numSeries);
        ...
        const { setDivisor, drawArraysInstanced: drawInstanced } = instancing;
        ...
        // One dispatch per series. Each series occupies a contiguous
        // `[s*cap, s*cap + count[s])` slice of the slotted buffer, so the
        // start-vs-end stride trick (end = start + stride) works without
        // straddling boundaries. Rebinding per series shifts the byte
        // offset so instance 0 is the series' first segment.
        ...
        gl.enableVertexAttribArray(cache.a_series_start);
        setDivisor(cache.a_series_start, 1);
        gl.enableVertexAttribArray(cache.a_series_end);
        setDivisor(cache.a_series_end, 1);
        ...
        setDivisor(cache.a_series_start, 0);
        setDivisor(cache.a_series_end, 0);
    }
